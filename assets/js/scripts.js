...
(function (menuConfig) {
	...
	function initSubmenuPositions() {
		...
		for (var i = 0; i < submenuParents.length; i++) {
			...
			submenuParents[i].addEventListener(eventTrigger, function () {
				...
				if (
					this.parentNode ===
					document.querySelector(config.menuSelector)
				) {
					widthMultiplier = 1;
				}
				...
				if (config.submenuWidth !== 'auto') {
					...
					if (window.innerWidth < submenuPotentialPosition) {
						...
						submenu.classList.remove(
							config.submenuRightPositionClass
						);
					...
					if (widthMultiplier === 1) {
						submenuPotentialPosition =
							itemPosition + submenu.clientWidth;
					} else {
						submenuPotentialPosition =
							itemPosition +
							this.clientWidth +
	...
	function initMobileMenuOverlay() {
		...
		var menuContentHTML = document.querySelector(
			config.menuSelector
		).outerHTML;
	...
	function initToggleSubmenu(menuWrapper) {
		...
		var parents = menuWrapper.querySelectorAll(
			'.' + config.parentItemClass
		);
				...
				if (childNodes[j].tagName === 'A') {
					childNodes[j].addEventListener('click', function (e) {
						var lastClick = parseInt(
							this.getAttribute('data-last-click'),
							10
						);
						...
						if (isNaN(lastClick)) {
							...
							lastClick + config.doubleClickTime <=
							currentTime
						) {
							e.preventDefault();
							this.setAttribute('data-last-click', currentTime);
	...
	function initClosingMenuOnClickLink() {
		...
		for (var i = 0; i < links.length; i++) {
			if (
				links[i].parentNode.classList.contains(config.parentItemClass)
			) {
				continue;
			}
	...
	function closeMenu(clickedLink, forceClose) {
		if (forceClose === false) {
			if (
				clickedLink.parentNode.classList.contains(
					config.parentItemClass
				)
			) {
				return;
